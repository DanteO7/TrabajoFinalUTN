using AutoMapper;
using backend_proyecto.Models;
using backend_proyecto.Models.DTOs;
using backend_proyecto.Repositories;
using backend_proyecto.Services;
using backend_proyecto.Utils.Errors;
using Moq;
using System.Net;

namespace UnitTests.Services.Class;

public class CreateClassShould
{
    private readonly Mock<IClassRepository> _classRepoMock;
    private readonly Mock<ITenantRepository> _tenantRepoMock;
    private readonly Mock<IActivityRepository> _activityRepoMock;
    private readonly Mock<IProfessorRepository> _professorRepoMock;
    private readonly Mock<IReservationRepository> _reservationRepoMock;
    private readonly Mock<IMapper> _mapperMock;

    private readonly ClassServices _classServices;

    public CreateClassShould()
    {
        _classRepoMock = new Mock<IClassRepository>();
        _tenantRepoMock = new Mock<ITenantRepository>();
        _activityRepoMock = new Mock<IActivityRepository>();
        _professorRepoMock = new Mock<IProfessorRepository>();
        _reservationRepoMock = new Mock<IReservationRepository>();
        _mapperMock = new Mock<IMapper>();

        _classServices = new ClassServices(
            _classRepoMock.Object,
            _tenantRepoMock.Object,
            _mapperMock.Object,
            _activityRepoMock.Object,
            _professorRepoMock.Object,
            _reservationRepoMock.Object
        );
    }

    private backend_proyecto.Models.Activity ValidActivity() =>
        new backend_proyecto.Models.Activity
        {
            Id = 1,
            Name = "Yoga",
            TenantId = 1
        };

    private backend_proyecto.Models.Professor ValidProfessor() =>
        new backend_proyecto.Models.Professor
        {
            Id = 1,
            UserId = 1,
            TenantId = 1
        };

    private backend_proyecto.Models.Tenant ValidTenant() =>
        new backend_proyecto.Models.Tenant
        {
            Id = 1,
            Name = "Gym"
        };

    private CreateClassDTO ValidDto() =>
        new CreateClassDTO
        {
            ActivityId = 1,
            ProfessorId = 1,
            TenantId = 1,
            Date = DateTime.Now.AddDays(1),
            StartTime = new TimeOnly(10, 0),
            EndTime = new TimeOnly(11, 0),
            MaxCapacity = 20
        };

    private backend_proyecto.Models.Class ValidClass() =>
        new backend_proyecto.Models.Class
        {
            Id = 1,
            ActivityId = 1,
            ProfessorId = 1,
            TenantId = 1,
            Date = DateTime.Now.AddDays(1),
            StartTime = new TimeOnly(10, 0),
            EndTime = new TimeOnly(11, 0),
            MaxCapacity = 20
        };

    private ResponseClassDTO ValidResponseDto() =>
        new ResponseClassDTO
        {
            Id = 1,
            ActivityId = 1,
            ProfessorId = 1,
            TenantId = 1,
            MaxCapacity = 20
        };

    [Fact]
    public async Task CreateClass_WhenDataIsValid()
    {
        // Arrange

        var dto = ValidDto();
        var classEntity = ValidClass();
        var responseDto = ValidResponseDto();

        _activityRepoMock
            .Setup(r => r.GetOneAsync(
                It.IsAny<System.Linq.Expressions.Expression<System.Func<backend_proyecto.Models.Activity, bool>>>()))
            .ReturnsAsync(ValidActivity());

        _professorRepoMock
            .Setup(r => r.GetOneAsync(
                It.IsAny<System.Linq.Expressions.Expression<System.Func<backend_proyecto.Models.Professor, bool>>>()))
            .ReturnsAsync(ValidProfessor());

        _tenantRepoMock
            .Setup(r => r.GetOneAsync(
                It.IsAny<System.Linq.Expressions.Expression<System.Func<backend_proyecto.Models.Tenant, bool>>>()))
            .ReturnsAsync(ValidTenant());

        _classRepoMock
            .Setup(r => r.ExistsScheduleConflict(dto, null))
            .ReturnsAsync(false);

        _mapperMock
            .Setup(m => m.Map<backend_proyecto.Models.Class>(dto))
            .Returns(classEntity);

        _classRepoMock
            .Setup(r => r.CreateOneAsync(It.IsAny<backend_proyecto.Models.Class>()))
            .Returns(Task.CompletedTask);

        _classRepoMock
            .Setup(r => r.GetOneAsync(
                It.IsAny<System.Linq.Expressions.Expression<System.Func<backend_proyecto.Models.Class, bool>>>(),
                It.IsAny<System.Linq.Expressions.Expression<System.Func<backend_proyecto.Models.Class, object>>[]>()))
            .ReturnsAsync(classEntity);

        _mapperMock
            .Setup(m => m.Map<ResponseClassDTO>(classEntity))
            .Returns(responseDto);

        // Act

        var result = await _classServices.CreateOne(dto);

        // Assert

        Assert.NotNull(result);

        _classRepoMock.Verify(
            r => r.CreateOneAsync(It.IsAny<backend_proyecto.Models.Class>()),
            Times.Once);
    }

    [Fact]
    public async Task ThrowError_WhenActivityDoesNotExist()
    {
        // Arrange

        var dto = ValidDto();

        _activityRepoMock
            .Setup(r => r.GetOneAsync(
                It.IsAny<System.Linq.Expressions.Expression<System.Func<backend_proyecto.Models.Activity, bool>>>()))
            .ReturnsAsync((backend_proyecto.Models.Activity?)null);

        // Act

        var ex = await Assert.ThrowsAsync<HttpResponseError>(() =>
            _classServices.CreateOne(dto));

        // Assert

        Assert.Equal(HttpStatusCode.NotFound, ex.StatusCode);

        Assert.Equal(
            "No se encontró una actividad con el Id = '1'",
            ex.Message);
    }

    [Fact]
    public async Task ThrowError_WhenStartTimeIsGreaterThanEndTime()
    {
        // Arrange

        var dto = ValidDto();

        dto.StartTime = new TimeOnly(12, 0);
        dto.EndTime = new TimeOnly(10, 0);

        _activityRepoMock
            .Setup(r => r.GetOneAsync(
                It.IsAny<System.Linq.Expressions.Expression<System.Func<backend_proyecto.Models.Activity, bool>>>()))
            .ReturnsAsync(ValidActivity());

        _professorRepoMock
            .Setup(r => r.GetOneAsync(
                It.IsAny<System.Linq.Expressions.Expression<System.Func<backend_proyecto.Models.Professor, bool>>>()))
            .ReturnsAsync(ValidProfessor());

        _tenantRepoMock
            .Setup(r => r.GetOneAsync(
                It.IsAny<System.Linq.Expressions.Expression<System.Func<backend_proyecto.Models.Tenant, bool>>>()))
            .ReturnsAsync(ValidTenant());

        // Act

        var ex = await Assert.ThrowsAsync<HttpResponseError>(() =>
            _classServices.CreateOne(dto));

        // Assert

        Assert.Equal(HttpStatusCode.BadRequest, ex.StatusCode);
    }

    [Fact]
    public async Task ThrowError_WhenProfessorHasScheduleConflict()
    {
        // Arrange

        var dto = ValidDto();

        _activityRepoMock
            .Setup(r => r.GetOneAsync(
                It.IsAny<System.Linq.Expressions.Expression<System.Func<backend_proyecto.Models.Activity, bool>>>()))
            .ReturnsAsync(ValidActivity());

        _professorRepoMock
            .Setup(r => r.GetOneAsync(
                It.IsAny<System.Linq.Expressions.Expression<System.Func<backend_proyecto.Models.Professor, bool>>>()))
            .ReturnsAsync(ValidProfessor());

        _tenantRepoMock
            .Setup(r => r.GetOneAsync(
                It.IsAny<System.Linq.Expressions.Expression<System.Func<backend_proyecto.Models.Tenant, bool>>>()))
            .ReturnsAsync(ValidTenant());

        _classRepoMock
            .Setup(r => r.ExistsScheduleConflict(dto, null))
            .ReturnsAsync(true);

        // Act

        var ex = await Assert.ThrowsAsync<HttpResponseError>(() =>
            _classServices.CreateOne(dto));

        // Assert

        Assert.Equal(HttpStatusCode.BadRequest, ex.StatusCode);

        Assert.Equal(
            "El profesor ya tiene una clase en ese horario",
            ex.Message);
    }
}