...
namespace backend_proyecto.Services
{
    public class ClassServices
    {
        ...
        public ClassServices(IClassRepository classRepository, ITenantRepository tenantRepository, IMapper mapper, IActivityRepository activityRepository, IProfessorRepository professorRepository)
        {
        ...
        public async Task<List<Class>> GetClassesByDate(int tenantId, DateTime date)
        {
            ...
            return await _classRepository.GetAllAsync(c => c.TenantId == tenantId && c.Date.Date == date.Date);
        }

        public async Task<Class> CreateOne(CreateClassDTO createClassDTO)
        {
            ...
            var conflict = await _classRepository.ExistsScheduleConflict(createClassDTO);
            if (conflict)
            {
        ...
        public async Task<Class> UpdateOne(int id, UpdateClassDTO updateClassDTO)
        {
            var classEntity = await _classRepository.GetOneAsync(c => c.Id == id);
            if (classEntity == null)
            {
                throw new HttpResponseError(HttpStatusCode.NotFound, $"No se encontró una clase con el Id = '{id}'");
            }
                ...
                updateClassDTO.EndTime != null)
            {
                ...
                var conflict = await _classRepository.ExistsScheduleConflict(dtoForValidation);
