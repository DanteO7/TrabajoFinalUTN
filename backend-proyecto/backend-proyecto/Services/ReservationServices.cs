...
namespace backend_proyecto.Services
{
    public class ReservationServices
    {
        ...
        public async Task<List<ResponseReservationDTO>> CreateMultiple(
            BulkCreateReservationDTO bulkDTO)
        {
            ...
            if (
                bulkDTO.StudentIds == null ||
                bulkDTO.StudentIds.Count == 0
            )
            {
                throw new HttpResponseError(
                    HttpStatusCode.BadRequest,
            ...
            foreach (var studentId in bulkDTO.StudentIds)
            {
                ...
                var existingReservation =
                    await _reservationRepository.GetOneAsync(
                        r =>
            ...
            {
                await _reservationRepository.CreateOneAsync(
                    reservation
                );
            }

            return _mapper.Map<List<ResponseReservationDTO>>(
                reservations
            );
        }
        ...
        private async Task UpdateCompletedReservations()
        {
            ...
            var reservations =
                await _reservationRepository.Query()
                    .Include(r => r.Class)
                    .Where(r =>
                        r.ReservationStatus !=
        ...
        public async Task<ResponseReservationDTO> GetById(int id)
        {
            ...
            return _mapper.Map<ResponseReservationDTO>(
                reservation
            );
        }

        public async Task<List<ResponseReservationDTO>> GetByClassId(
            int classId)
        {
            await _permissionServices.CheckPermission(
                Permissions.RESERVATION_READ
        ...
        public async Task<ResponseReservationDTO> GetByClassAndStudent(
            int classId,
            int studentId)
        {
            await _permissionServices.CheckPermission(
        ...
        public async Task<List<ResponseReservationDTO>> GetByStudentId(
            int studentId)
        {
            await _permissionServices.CheckPermission(
                Permissions.RESERVATION_READ
        ...
        public async Task DeleteOne(int id)
        {
            ...
            await _reservationRepository.DeleteOneAsync(
                reservation
            );

            if (wasFull)
            {
                await _waitlistSubject.Notify(
